...
const Class = () => {
  ...
  const fetchData = () => {
    axios.get(`${baseUrl}/settings/levels`, { headers })
      .then(response => {
  ...
  const createNew = () => {
    document.getElementById('modal-title').innerText = "Creating New Class";
    $('#btn-edit').hide();
    $('#btn-save').show();
  ...
  $(document).on('click', '.delete-btn', function (e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post(`${baseUrl}/settings/levels/delete`, { level_id: id }, { headers })
          .then(response => {
  ...
  const sendSaveRequest = () => {
    ...
    axios.post(`${baseUrl}/settings/levels`, payload, { headers })
      .then(response => {
        const data = response.data;
        if (data.success) {
          Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
  ...
  const sendEditRequest = () => {
    const title = document.getElementById('role').value;
    const description = document.getElementById('description').value;
    const level_id = document.getElementById('gottenId').value;
    ...
    const payload = { level_id, title, description };

    // Send POST request to the API using axios
    axios.post(`${baseUrl}/settings/levels/update`, payload, { headers })
      .then(response => {
        const data = response.data;
        if (data.success) {
          Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
  ...
  const getReadyListRemove = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post(`${baseUrl}/universal/bulk-remove`, { ids: ids, table:"levels" }, { headers })
          .then(response => {
            const data = response.data;
            if (data.success) {
              Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
            ...
            <div className="modal-body">
              ...
              <div className="card-block">
                ...
                <div className="col-md-12">
                  <label htmlFor="role" className="form-label">Title of the class <span className='required'>*</span></label>
                  <input type="text" id='role' className='form-control' placeholder='eg. Basic 1' />
                </div>
