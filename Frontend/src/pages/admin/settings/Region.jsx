...
const Region = () => {
  ...
  // Fetch THE LIST OF THE COUNTRIES
  const fetchCountries = () => {
    axios.get(`${baseUrl}/settings/countries`, { headers })
      .then(response => {
        const fetchedData = response.data;
        if (fetchedData.success) {
          const sortedCountries = fetchedData.data.sort((a, b) => {
      ...
      columns: [

        {
          data: null, render: function (data) {
            return `<input type="checkbox" class="row-select" data-title="${data.region}" data-id="${data.id}" />`;
          }
        },

        { data: 'country.country' },
        { data: 'region', width: '100%' },
        { data: 'created_at' },
        { data: 'status' },
        {
          data: null, render: function (data) {
              ...
              <div class="btn-group dropdown">
                  ...
                  <div class="dropdown-menu dropdown-menu-right">
                      <a class="dropdown-item edit-btn" 
                        href="#" data-title="${data.region}" data-country_id="${data.country_id}" data-id="${data.id}">
                      ...
                      <a class="dropdown-item text-danger delete-btn" href="#" data-title="${data.region}" data-id="${data.id}"><i class="mdi mdi-delete-outline mr-1 font-18 vertical-middle"></i>Remove Record</a>
                  </div>
              </div>
            `;
          }
  ...
  const createNew = () => {
    document.getElementById('modal-title').innerText = "Creating New Region";
    $('#btn-edit').hide();
    $('#btn-save').show();
  ...
  useEffect(() => {
    const handleEditClick = (e) => {
      ...
      const country_id = button.data('country_id');
      const region = button.data('title');
      const id = button.data('id');

      // Set the values into the input fields
      document.querySelector('#country_id').value = country_id;
      document.querySelector('#country_id').dispatchEvent(new Event('change'));
      document.getElementById('region').value = region;
      document.getElementById('gottenId').value = id;

      // Update modal title and show it
      document.getElementById('modal-title').innerText = "Modifying " + region;
      $('#btn-save').hide();
      $('#btn-edit').show();
  ...
  $(document).on('click', '.delete-btn', function (e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post(`${baseUrl}/settings/regions/delete`, { region_id: id }, { headers })
          .then(response => {
  ...
  const sendSaveRequest = () => {
    ...
    const payload = { country_id, region };

    // Send POST request to the API using axios
    axios.post(`${baseUrl}/settings/regions`, payload, { headers })
      .then(response => {
        const data = response.data;
        if (data.success) {
          Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
  ...
  const sendEditRequest = () => {
    const country_id = document.getElementById('country_id').value;
    const region = document.getElementById('region').value;
    const region_id = document.getElementById('gottenId').value;
    ...
    const payload = { region_id, country_id, region };

    // Send POST request to the API using axios
    axios.post(`${baseUrl}/settings/regions/update`, payload, { headers })
      .then(response => {
        const data = response.data;
        if (data.success) {
          Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
  ...
  const getReadyListRemove = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post(`${baseUrl}/universal/bulk-remove`, { ids: ids, table:"regions" }, { headers })
          .then(response => {
            const data = response.data;
            if (data.success) {
              Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
            ...
            <div className="modal-body">
              ...
              <div className="card-block">
                ...
                <div className="col-md-12">
                  <label htmlFor="buyerName" className="form-label">Select Country <span className='required'>*</span></label>
                  <select className="form-control select2" id="country_id" required>
                    <option value="">Choose...</option>
                    {countries.map(country => (<option key={country.id} value={country.id}> {country.country.toUpperCase()} </option>))}
                  </select>
                </div>
                <div className="col-md-12 mt-2">
                  <label htmlFor="phone" className="form-label">Region / State title</label>
                  <input type="text" className="form-control" id="region" placeholder="eg. Central Region" />
                </div>
              </div>
            </div>
