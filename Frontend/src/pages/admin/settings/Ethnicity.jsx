...
const Ethnicity = () => {
  ...
  const fetchData = () => {
    axios.get(`${baseUrl}/settings/ethnicities`, { headers })
      .then(response => {
  ...
  // Fetch THE LIST OF THE REGIONS
  const fetchRegions = () => {
    axios.get(`${baseUrl}/settings/regions`, { headers })
      .then(response => {
        const fetchedData = response.data;
        if (fetchedData.success) {
          const sortedRegions = fetchedData.data.sort((a, b) => {
      ...
      columns: [

        {
          data: null, render: function (data) {
            return `<input type="checkbox" class="row-select" data-title="${data.ethnicity}" data-id="${data.id}" />`;
          }
        },
              ...
              <div class="btn-group dropdown">
                  ...
                  <div class="dropdown-menu dropdown-menu-right">
                      <a class="dropdown-item edit-btn" 
                        href="#" data-title="${data.ethnicity}" data-description="${data.description}" data-region_id="${data.region_id}" data-id="${data.id}">
                      ...
                      <a class="dropdown-item text-danger delete-btn" href="#" data-title="${data.ethnicity}" data-id="${data.id}"><i class="mdi mdi-delete-outline mr-1 font-18 vertical-middle"></i>Remove Record</a>
                  </div>
              </div>
            `;
          }
  ...
  const createNew = () => {
    document.getElementById('modal-title').innerText = "Creating New Ethnicity";
    $('#btn-edit').hide();
    $('#btn-save').show();
  ...
  useEffect(() => {
    const handleEditClick = (e) => {
      ...
      const region_id = button.data('region_id');
      const ethnicity = button.data('title');
      const description = button.data('description');
      const id = button.data('id');

      // Set the values into the input fields
      document.querySelector('#region_id').value = region_id;
      document.querySelector('#region_id').dispatchEvent(new Event('change'));
      ...
      document.getElementById('modal-title').innerText = "Modifying " + ethnicity;
      $('#btn-save').hide();
      $('#btn-edit').show();
  ...
  $(document).on('click', '.delete-btn', function (e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post(`${baseUrl}/settings/ethnicities/delete`, { ethnicity_id: id }, { headers })
          .then(response => {
  ...
  const sendSaveRequest = () => {
    ...
    if (!region_id || !ethnicity || !description) {
    ...
    const payload = { region_id, ethnicity, description };

    // Send POST request to the API using axios
    axios.post(`${baseUrl}/settings/ethnicities`, payload, { headers })
      .then(response => {
        const data = response.data;
        if (data.success) {
          Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
  ...
  const sendEditRequest = () => {
    const region_id = document.getElementById('region_id').value;
    const ethnicity = document.getElementById('ethnicity').value;
    const ethnicity_id = document.getElementById('gottenId').value;
    const description = document.getElementById('description').value;


    if (!ethnicity_id || !ethnicity || !description) {
      Swal.fire({ icon: 'warning', title: 'Attention Please', text: 'Please fill all the fields and it is required', });
      return;
    }
    ...
    const payload = { ethnicity_id, region_id, ethnicity, description };

    // Send POST request to the API using axios
    axios.post(`${baseUrl}/settings/ethnicities/update`, payload, { headers })
      .then(response => {
        const data = response.data;
        if (data.success) {
          Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
  ...
  const getReadyListRemove = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post(`${baseUrl}/universal/bulk-remove`, { ids: ids, table:"ethnicities" }, { headers })
          .then(response => {
            const data = response.data;
            if (data.success) {
              Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
            ...
            <div className="modal-body">
              ...
              <div className="card-block">
                ...
                <div className="col-md-12">
                  <label htmlFor="buyerName" className="form-label">State / Municipal / Province for ethnicity <span className='required'>*</span></label>
                  <select className="form-control select2" id="region_id" required>
                    <option value="">Choose...</option>
                    {regions.map(region => (<option key={region.id} value={region.id}> {region.region.toUpperCase()} </option>))}
                  </select>
                </div>
                <div className="col-md-12 mt-2">
                  <label htmlFor="phone" className="form-label">Ethnicity title</label>
                  <input type="text" className="form-control" id="ethnicity" placeholder="eg. Guan" />
                </div>
                <div className="col-md-12 mt-2">
