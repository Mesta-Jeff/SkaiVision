...
const District = () => {
  ...
  const fetchData = () => {
    axios.get(`${baseUrl}/settings/districts`, { headers })
      .then(response => {
      ...
      columns: [

        {
          data: null, render: function (data) {
            return `<input type="checkbox" class="row-select" data-title="${data.district}" data-id="${data.id}" />`;
          }
        },

        { data: 'region.region' },
        { data: 'district', width: '100%' },
        { data: 'created_at' },
        { data: 'status' },
        {
          data: null, render: function (data) {
              ...
              <div class="btn-group dropdown">
                  ...
                  <div class="dropdown-menu dropdown-menu-right">
                      <a class="dropdown-item edit-btn" 
                        href="#" data-title="${data.district}" data-region_id="${data.region_id}" data-id="${data.id}">
                      ...
                      <a class="dropdown-item text-danger delete-btn" href="#" data-title="${data.district}" data-id="${data.id}"><i class="mdi mdi-delete-outline mr-1 font-18 vertical-middle"></i>Remove Record</a>
                  </div>
              </div>
            `;
          }
  ...
  const createNew = () => {
    document.getElementById('modal-title').innerText = "Creating New District";
    $('#btn-edit').hide();
    $('#btn-save').show();
  ...
  useEffect(() => {
    const handleEditClick = (e) => {
      ...
      const district = button.data('title');
      ...
      document.getElementById('modal-title').innerText = "Modifying " + district;
      $('#btn-save').hide();
      $('#btn-edit').show();
  ...
  $(document).on('click', '.delete-btn', function (e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post(`${baseUrl}/settings/districts/delete`, { district_id: id }, { headers })
          .then(response => {
  ...
  const sendSaveRequest = () => {
    ...
    const payload = { region_id, district };

    // Send POST request to the API using axios
    axios.post(`${baseUrl}/settings/districts`, payload, { headers })
      .then(response => {
        const data = response.data;
        if (data.success) {
          Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
  ...
  const sendEditRequest = () => {
    const region_id = document.getElementById('region_id').value;
    const district = document.getElementById('district').value;
    const district_id = document.getElementById('gottenId').value;
    ...
    const payload = { district_id, region_id, district };

    // Send POST request to the API using axios
    axios.post(`${baseUrl}/settings/districts/update`, payload, { headers })
      .then(response => {
        const data = response.data;
        if (data.success) {
          Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
  ...
  const getReadyListRemove = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post(`${baseUrl}/universal/bulk-remove`, { ids: ids, table:"districts" }, { headers })
          .then(response => {
            const data = response.data;
            if (data.success) {
              Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
            ...
            <div className="modal-body">
              ...
              <div className="card-block">
                ...
                <div className="col-md-12">
                  <label htmlFor="buyerName" className="form-label">District / Municipal is under which region <span className='required'>*</span></label>
                ...
                <div className="col-md-12 mt-2">
                  <label htmlFor="phone" className="form-label">District / Municipal / Province title</label>
                  <input type="text" className="form-control" id="district" placeholder="eg. Central Region" />
                </div>
              </div>
            </div>
