...
const AccreditationBody = () => {
  ...
  useEffect(() => {
    const titles = "Accreditation Bodies";
    setPageTitle(titles);
  ...
  const fetchData = () => {
    axios.get(`${baseUrl}/settings/accreditation-bodies`, { headers })
      .then(response => {
      ...
      columns: [

        {
          data: null, render: function (data) {
            return `<input type="checkbox" class="row-select" data-title="${data.name}" data-id="${data.id}" />`;
          }
        },
              ...
              <div class="btn-group dropdown">
                  ...
                  <div class="dropdown-menu dropdown-menu-right">
                      <a class="dropdown-item edit-btn" 
                        href="#" data-title="${data.name}" data-description="${data.description}" data-id="${data.id}">
                      ...
                      <a class="dropdown-item text-danger delete-btn" href="#" data-title="${data.name}" data-id="${data.id}"><i class="mdi mdi-delete-outline mr-1 font-18 vertical-middle"></i>Remove Record</a>
                  </div>
              </div>
            `;
          }
  ...
  const createNew = () => {
    document.getElementById('modal-title').innerText = "Creating New Accreditation Body";
    $('#btn-edit').hide();
    $('#btn-save').show();
  ...
  $(document).on('click', '.delete-btn', function (e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post(`${baseUrl}/settings/accreditation-bodies/delete`, { accreditation_id: id }, { headers })
          .then(response => {
  ...
  const sendSaveRequest = () => {
    ...
    axios.post(`${baseUrl}/settings/accreditation-bodies`, payload, { headers })
      .then(response => {
        const data = response.data;
        if (data.success) {
          Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
  ...
  const sendEditRequest = () => {
    const title = document.getElementById('role').value;
    const description = document.getElementById('description').value;
    const accreditation_id = document.getElementById('gottenId').value;
    ...
    const payload = { accreditation_id, name: title, description };

    // Send POST request to the API using axios
    axios.post(`${baseUrl}/settings/accreditation-bodies/update`, payload, { headers })
      .then(response => {
        const data = response.data;
        if (data.success) {
          Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
  ...
  const getReadyListRemove = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post(`${baseUrl}/universal/bulk-remove`, { ids: ids, table:"accreditation_bodies" }, { headers })
          .then(response => {
            const data = response.data;
            if (data.success) {
              Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
            ...
            <div className="modal-body">
              ...
              <div className="card-block">
                ...
                <div className="col-md-12">
                  <label htmlFor="role" className="form-label">Accreditation body name <span className='required'>*</span></label>
                  <input type="text" id='role' className='form-control' placeholder='eg. GES' />
                </div>
