...
const Relationship = () => {
  ...
  const fetchData = () => {
    axios.get(`${baseUrl}/settings/relationships`, { headers })
      .then(response => {
  ...
  const createNew = () => {
    document.getElementById('modal-title').innerText = "Creating New Relationship";
    $('#btn-edit').hide();
    $('#btn-save').show();
  ...
  $(document).on('click', '.delete-btn', function (e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post(`${baseUrl}/settings/relationships/delete`, { relationship_id: id }, { headers })
          .then(response => {
  ...
  const sendSaveRequest = () => {
    ...
    axios.post(`${baseUrl}/settings/relationships`, payload, { headers })
      .then(response => {
        const data = response.data;
        if (data.success) {
          Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
  ...
  const sendEditRequest = () => {
    const title = document.getElementById('role').value;
    const description = document.getElementById('description').value;
    const relationship_id = document.getElementById('gottenId').value;
    ...
    const payload = { relationship_id, title, description };

    // Send POST request to the API using axios
    axios.post(`${baseUrl}/settings/relationships/update`, payload, { headers })
      .then(response => {
        const data = response.data;
        if (data.success) {
          Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
  ...
  const getReadyListRemove = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post(`${baseUrl}/universal/bulk-remove`, { ids: ids, table:"relationships" }, { headers })
          .then(response => {
            const data = response.data;
            if (data.success) {
              Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
            ...
            <div className="modal-body">
              ...
              <div className="card-block">
                ...
                <div className="col-md-12">
                  <label htmlFor="role" className="form-label">Title of the relationship<span className='required'>*</span></label>
                  <input type="text" id='role' className='form-control' placeholder='eg. Brother' />
                </div>
