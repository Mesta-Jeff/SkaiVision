...
const Religion = () => {
  ...
  const fetchData = () => {
    axios.get(`${baseUrl}/settings/religions`, { headers })
      .then(response => {
      ...
      columns: [

        {
          data: null, render: function (data) {
            return `<input type="checkbox" class="row-select" data-title="${data.religion}" data-id="${data.id}" />`;
          }
        },
              ...
              <div class="btn-group dropdown">
                  ...
                  <div class="dropdown-menu dropdown-menu-right">
                      <a class="dropdown-item edit-btn" 
                        href="#" data-title="${data.religion}" data-description="${data.description}" data-id="${data.id}">
                      ...
                      <a class="dropdown-item text-danger delete-btn" href="#" data-title="${data.religion}" data-id="${data.id}"><i class="mdi mdi-delete-outline mr-1 font-18 vertical-middle"></i>Remove Record</a>
                  </div>
              </div>
            `;
          }
  ...
  const createNew = () => {
    document.getElementById('modal-title').innerText = "Creating New Religion";
    $('#btn-edit').hide();
    $('#btn-save').show();
  ...
  $(document).on('click', '.delete-btn', function (e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post(`${baseUrl}/settings/religions/delete`, { religion_id: id }, { headers })
          .then(response => {
  ...
  const sendSaveRequest = () => {
    ...
    const payload = { religion: title, description };

    // Send POST request to the API using axios
    axios.post(`${baseUrl}/settings/religions`, payload, { headers })
      .then(response => {
        const data = response.data;
        if (data.success) {
          Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
  ...
  const sendEditRequest = () => {
    const title = document.getElementById('role').value;
    const description = document.getElementById('description').value;
    const religion_id = document.getElementById('gottenId').value;
    ...
    const payload = { religion_id, religion: title, description };

    // Send POST request to the API using axios
    axios.post(`${baseUrl}/settings/religions/update`, payload, { headers })
      .then(response => {
        const data = response.data;
        if (data.success) {
          Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
  ...
  const getReadyListRemove = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post(`${baseUrl}/universal/bulk-remove`, { ids: ids, table:"religions" }, { headers })
          .then(response => {
            const data = response.data;
            if (data.success) {
              Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
            ...
            <div className="modal-body">
              ...
              <div className="card-block">
                ...
                <div className="col-md-12">
                  <label htmlFor="role" className="form-label">Title of the religion <span className='required'>*</span></label>
                  <input type="text" id='role' className='form-control' placeholder='eg. Islamic' />
                </div>
