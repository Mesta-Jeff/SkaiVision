...
const SchoolType = () => {
  ...
  const fetchData = () => {
    axios.get(`${baseUrl}/settings/school-types`, { headers })
      .then(response => {
  ...
  const createNew = () => {
    document.getElementById('modal-title').innerText = "Creating New School Type";
    $('#btn-edit').hide();
    $('#btn-save').show();
  ...
  $(document).on('click', '.delete-btn', function (e) {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post(`${baseUrl}/settings/school-types/delete`, { school_type_id: id }, { headers })
          .then(response => {
  ...
  const sendSaveRequest = () => {
    ...
    axios.post(`${baseUrl}/settings/school-types`, payload, { headers })
      .then(response => {
        const data = response.data;
        if (data.success) {
          Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
  ...
  const sendEditRequest = () => {
    const title = document.getElementById('role').value;
    const description = document.getElementById('description').value;
    const school_type_id = document.getElementById('gottenId').value;
    ...
    const payload = { school_type_id, title, description };

    // Send POST request to the API using axios
    axios.post(`${baseUrl}/settings/school-types/update`, payload, { headers })
      .then(response => {
        const data = response.data;
        if (data.success) {
          Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
  ...
  const getReadyListRemove = () => {
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.post(`${baseUrl}/universal/bulk-remove`, { ids: ids, table:"school_types" }, { headers })
          .then(response => {
            const data = response.data;
            if (data.success) {
              Swal.fire({ icon: 'success', title: 'Success!', text: data.message, }).then(() => {
            ...
            <div className="modal-body">
              ...
              <div className="card-block">
                ...
                <div className="col-md-12">
                  <label htmlFor="buyerName" className="form-label">Name of the school type you want to create <span className='required'>*</span></label>
                  <input type="text" id='role' className='form-control' placeholder='eg. Junior High School' />
                </div>
